...
#Important: for binary responses plinomial should be fit
models_list_single_1 = list()
for (g in 1:ncol(h1_data[,-1])){

  reduced_1 = lm(pval ~ h1_data[,g], data = h1_data)
...
model_mt <- glm(pval ~ ans_mt_h1, data = h1_data, family = "binomial")
summary(model_mt)
